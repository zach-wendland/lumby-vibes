...
import type { EnemyData, SpawnLocation, AttackStyle, CombatStats, XPRewards } from '../types/index';

/**
 * Complete enemy definitions for Lumbridge area
 */
 ...
 */
export const SPAWN_LOCATIONS: Record<string, SpawnLocation[]> = {
    CHICKEN: [
 ...
 * @param enemyId - Enemy identifier (case-insensitive)
 * @returns Enemy data or undefined if not found
 */
...
/**
 * Get all enemies that spawn in a specific location
 * @param location - Location/area name
 * @returns Array of enemy types and their spawn data for that location
 */
export function getEnemiesByLocation(location: string): Array<{ type: string } & SpawnLocation> {
    const enemies: Array<{ type: string } & SpawnLocation> = [];
...
/**
 * Get all enemy IDs
 * @returns Array of all enemy identifiers
 */
...
/**
 * Check if an enemy is aggressive
 * @param enemyId - Enemy identifier
 * @returns True if enemy is aggressive
 */
export function isEnemyAggressive(enemyId: string): boolean {
    const enemy = getEnemyData(enemyId);
