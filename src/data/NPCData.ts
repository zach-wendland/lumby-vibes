...
/**
 * Complete NPC definitions for Lumbridge area
 */
 ...
 * @param name - NPC name to search for
 * @returns NPC data or null if not found
 */
export function getNPCData(name: string): NPCData | null {
...
/**
 * Get NPC data by key
 * @param key - NPC key (e.g., 'DUKE_HORACIO')
 * @returns NPC data or undefined if not found
 */
export function getNPCDataByKey(key: string): NPCData | undefined {
    return NPC_DATA[key.toUpperCase()];
}

/**
 * Get all NPCs that give quests
 * @returns Array of NPC data for quest-giving NPCs
 */
export function getQuestGivers(): NPCData[] {
    return Object.values(NPC_DATA).filter(npc =>
        npc.questGiver && Array.isArray(npc.questGiver) && npc.questGiver.length > 0
    );
}

/**
 * Get all NPCs with shops
 * @returns Array of NPC data for shopkeepers
 */
export function getShopkeepers(): NPCData[] {
    return Object.values(NPC_DATA).filter(npc =>
        typeof npc.shop === 'string'
    );
}

/**
 * Get all roaming NPCs
 * @returns Array of NPC data for roaming NPCs
 */
export function getRoamingNPCs(): NPCData[] {
    return Object.values(NPC_DATA).filter(npc => npc.roaming);
}

/**
 * Get all NPC keys
 * @returns Array of all NPC keys
 */
export function getAllNPCKeys(): string[] {
    return Object.keys(NPC_DATA);
}
