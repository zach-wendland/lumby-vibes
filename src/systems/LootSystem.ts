 ...
 */
type DropTable = Record<string, DropData>;
 ...
 * Loot statistics
 */
interface LootStatistics {
    totalKills: number;
    totalValue: number;
    averageValue: number;
    commonDrops: Map<string, number>;
    rareDrops: Array<{ item: string; quantity: number; timestamp: number }>;
}
...
export const DROP_TABLES: Record<string, DropTable> = {
    ...
    SHEEP: {
        ...
        wool: { rarity: RARITY.COMMON, quantity: 1, chance: 0.30 }
    },
...
/**
 * LootSystem class - Handles loot generation
 */
export class LootSystem {
    private lootHistory: LootHistoryEntry[];
     ...
     */
    generateLoot(enemyType: string): LootItem[] {
     ...
     */
    rollRareDropTable(): LootItem[] {
        const rareLoot: LootItem[] = [];
     ...
     */
    calculateLootValue(loot: LootItem[]): number {
        let totalValue = 0;
     ...
     * Get loot statistics
     */
    getLootStatistics(enemyType: string | null = null): LootStatistics {
     ...
     */
    formatLootMessage(loot: LootItem[]): string {
     ...
     */
    getDropTable(enemyType: string): DropTable | undefined {
        return DROP_TABLES[enemyType];
    }
}
